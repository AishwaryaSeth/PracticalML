...
```{r}
rawData_train <- read.csv("pml-training.csv", na.strings = c("NA", ""))
rawData_test<-read.csv("pml-testing.csv", na.strings = c("NA", ""))
```
...
```{r}
propNAs <- colMeans(is.na(rawData_test))
table(propNAs)
```
...
data <- rawData_train[idx]
datat<-rawData_test[idx]
# check
ncol(data)
```
...
idx <- grep("^X$|user_name|timestamp|window", names(data))
# check
length(idx)
# remove columns
data2 <- data[-idx]
...
Now, the dataset contains one outcome column (`classe`) and `r ncol(data) - 1L` feature columns. The function `createDataPartition` of the `caret` package is used to split the data into a training and a cross-validation data set. Here, 70% of the data goes into the training set.
...
Random forests were used for training, with cross validation to ensure correctness of the data
